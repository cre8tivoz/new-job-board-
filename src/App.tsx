...
function EmployerDashboard() {
  ...
  const load = () => api<{jobs: Job[]}>('/employer/jobs').then((data) => setJobs(data.jobs)).catch((err) => setError(err.message)); useEffect(() => { void load(); }, []);
...
function AdminReview() {
  const [jobs, setJobs] = useState<Job[]>([]); const [error, setError] = useState(''); const load = () => api<{jobs: Job[]}>('/admin/jobs').then((data) => setJobs(data.jobs)).catch((err) => setError(err.message)); useEffect(() => { void load(); }, []);
